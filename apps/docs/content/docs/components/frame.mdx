...
<Preview
  src="/previews/frame.jpg"
  alt="The Frame component, running in the example app."
  width={1179}
  height={974}
/>
...
### Rows that show a value

A label on the left and its current value on the right, muted. Give the label `flex-1` so the value is pushed to the trailing edge.

<Preview
  src="/previews/frame-rows-that-show-a-value.jpg"
  alt="Frame — Rows that show a value."
  width={1179}
  height={618}
/>
...
<Frame>
  ...
  <Frame.Panel>
    {preferences.map(([label, value]) => (
      <Frame.Row key={label}>
        <Text size="sm" className="flex-1">{label}</Text>
...
An avatar in `Frame.Media`, a name and an email in `Frame.Content`, and the role in `Frame.Actions`. The header's action can be a badge instead of text.

<Preview
  src="/previews/frame-a-member-list.jpg"
  alt="Frame — A member list."
  width={1179}
  height={778}
/>
...
<Frame>
  ...
  <Frame.Panel>
    {members.map((member) => (
      <Frame.Row key={member.email}>
        <Frame.Media>
          <Avatar size="sm" fallback={member.initials} />
        </Frame.Media>
        <Frame.Content>
          <Frame.Title>{member.name}</Frame.Title>
          <Frame.Description>{member.email}</Frame.Description>
        </Frame.Content>
        <Frame.Actions>
...
`Frame.Media` holds any small graphic at its own size. Here each row leads with a progress ring of the app's own.

<Preview
  src="/previews/frame-a-leading-graphic.jpg"
  alt="Frame — A leading graphic."
  width={1179}
  height={808}
/>
...
<Frame>
  ...
  <Frame.Panel>
    {usage.map((item) => (
      <Frame.Row key={item.label}>
        <Frame.Media>
          <UsageRing percent={item.percent} />
        </Frame.Media>
        <Frame.Content>
...
<Preview
  src="/previews/frame-a-row-that-would-not-fit.jpg"
  alt="Frame — A row that would not fit."
  width={1179}
  height={730}
/>
...
<Preview
  src="/previews/frame-a-row-of-chips-that-wraps.jpg"
  alt="Frame — A row of chips that wraps."
  width={1179}
  height={342}
/>
...
<Preview
  src="/previews/frame-rows-that-lead-somewhere.jpg"
  alt="Frame — Rows that lead somewhere."
  width={1179}
  height={616}
/>
...
<Preview
  src="/previews/frame-sections-inside-a-panel.jpg"
  alt="Frame — Sections inside a panel."
  width={1179}
  height={812}
/>
...
The band shapes what is put in it. The actions take equal widths and full pills without being asked — a row of equal decisions should not arrive with three different radii — and the footer is held in further than the panel, because a row running the full width of the shell reads as another edge of it.

For a widget that only reports, keep the default. A band with nothing in it is a margin.

<Preview
  src="/previews/frame-a-widget-that-asks-for-something.jpg"
  alt="Frame — A widget that asks for something."
  width={1179}
  height={676}
/>
...
<Frame variant="inset">
  ...
  <Frame.Footer>
    <Button variant="secondary" onPress={declineAll}>Decline all</Button>
    <Button onPress={acceptAll}>Accept all</Button>
  </Frame.Footer>
</Frame>
```

### A header in the band

Under `inset` the header sits in the band above the panel, the same way the footer sits in the band below it.

<Preview
  src="/previews/frame-a-header-in-the-band.jpg"
  alt="Frame — A header in the band."
  width={1179}
  height={814}
/>
...
<Preview
  src="/previews/frame-plain.jpg"
  alt="Frame — variant plain, inside a Card."
  width={1179}
  height={446}
  caption="plain"
/>
