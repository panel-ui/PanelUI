...
<Preview
  ...
  width={1179}
  height={1888}
  caption="Both sides of a conversation, a named turn with a timestamp, and grouped turns."
/>
...
```tsx
import { Message, Avatar, Button, Shimmer, BottomSheet } from 'panelui-native';
```
...
Give the turn an `onLongPress` and the whole row becomes a long-press target with the library's press feedback — the gesture a chat uses to surface per-message actions. The component only exposes the gesture; open a sheet or a popover from it.
...
<Message align="end" onLongPress={() => setOpen(true)}>
  <Message.Content>
    <Message.Bubble>
      <Message.BubbleContent>Long-press me for actions.</Message.BubbleContent>
    </Message.Bubble>
  </Message.Content>
</Message>
