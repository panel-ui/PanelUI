...
`onCollapsedChange` fires once each way, on the crossing rather than every frame — enough to swap a status bar style or reveal a control that only belongs on the bar.
