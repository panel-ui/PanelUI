...
Every native control is given an explicit height, and takes its width from
ordinary layout. None of them ask the platform to size the host for them.

That is not a stylistic choice. The platform measures its own content a frame
late, and again whenever that content changes — so a host left to work its own
size out renders at nothing on the first frame, collapsing against whatever
sits above it, and jumps into place on the first press. Asking for one axis
only does not avoid it. The answer is not to ask.
