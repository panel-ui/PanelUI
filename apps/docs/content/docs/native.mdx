...
Every native control is given an explicit height rather than left to size
itself on the vertical axis. The platform measures its own content a frame
late, so a host that has to discover its height renders at nothing on the
first frame — the control collapses against whatever sits above it — and jumps
into place on the first interaction.
