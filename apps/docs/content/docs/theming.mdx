...
| `chart-1` … `chart-5` | Chart series, in order of prominence |
...
`--color-chart-1` through `--color-chart-5` are a separate ramp from the status colours, because
they answer a different question. A status colour *means* something — this failed, this is fine.
A series colour only has to be told apart from the other four.

They are ordered by prominence: `chart-1` is the series the chart is about, and each theme starts
it on that family's own accent. `LineChart` takes `chart-1` by default and each further series
takes the next one, so putting every chart in an app on brand is five lines in your `global.css`
and nothing else.
